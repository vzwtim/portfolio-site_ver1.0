...
export default function YouTubeFeature({ videoId, channelUrl, uploadsHandle, title = "旧街道を、自転車でたどる。" }: YouTubeFeatureProps) {
  ...
  return (
    <article className={styles.videoFeature} aria-label="YouTubeで展開する旅の記録">
      <div className={styles.videoFrame}><iframe src={embedSource} title={`${title} — @${uploadsHandle}`} allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen loading="lazy" /></div>
